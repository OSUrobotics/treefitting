...
class BezierCyl3D:
    ...
    def copy(self, bezier_crv=None, b_compute_mesh=False):
        ...
        if bezier_crv is None:
            bezier_crv = BezierCyl3D(
                np.copy(self.pt1), np.copy(self.pt2), np.copy(self.pt3), self.start_radii, self.end_radii
            )
    ...
    def pt_axis(self, t):
        ...
        pts_axis = np.array(
            [self.pt1[i] * (1 - t) ** 2 + 2 * (1 - t) * t * self.pt2[i] + t**2 * self.pt3[i] for i in range(0, 3)]
        )
    ...
    def tangent_axis(self, t):
        ...
        @return 3d vec"""
        vec_axis = [
            2 * t * (self.pt1[i] - 2.0 * self.pt2[i] + self.pt3[i]) - 2 * self.pt1[i] + 2 * self.pt2[i]
            for i in range(0, 3)
        ]
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{__here__}/data/DebugImages"):
        os.mkdir(f"{__here__}/data/DebugImages")
    branch = BezierCyl3D(
        [506.5, 156.0, 0.0],
        [457.49999996771703, 478.9999900052037, 0.0],
        [521.5, 318.0, 0.0],
        start_radius=10.5,
        end_radius=8.25,
    )
