...
parser.add_argument('--rownum', type=int,  default=98)
...
#     print(image.shape)
    if len(masks)==0:
        return image,np.zeros((image.shape[0], image.shape[1]), dtype=np.uint8),-1
...
def main_test(test_num):
    ...
    # save_num=5
    # saved_folder_path="row{}_4".format(test_num)
    ...
    #     if info2!=28:continue
        # if data_sample!='1659633437282113314_07.png':continue
        print(data_sample)
        # print(info2)
        st=[]
        st.append(time.time())#0
        pc = np.load(pc_path)
        im = cv2.imread(img_path)

        st.append(time.time())#1
        outputs = predictor(im) 
        empty_img = np.zeros_like(im[:, :, ::-1])
        v = Visualizer(empty_img, metadata=my_dataset_val_metadata, scale=1.0,)
        vis_imgs2,mask,scores =test2(outputs["instances"].to("cpu"),0.2,v,im)

        st.append(time.time())#2
        if scores!=-1:
            a = PCAslice2(mask, pc, vis_imgs2)
            a.GetWidth()
        
        st.append(time.time())#3
        for j in range(3):
            lst_t[j].append(st[j+1]-st[j])
        print(truewidths[info2-1])
        w5[info2-1]=a.width5
    timenp=np.array(lst_t)
    timenp1=np.mean(timenp,axis=1)
    print(timenp.shape)
    print(timenp1)
    timenp2=timenp1#*1000
    #print(timenp2)
    print('{:.3f}&{:.3f}&{:.3f}&{:.3f}\\cr'.format(timenp2[0],timenp2[1],timenp2[2],np.sum(timenp2)))
